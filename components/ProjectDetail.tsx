...
export const ProjectDetail: React.FC = () => {
  ...
  if (!project || !project.fullDetails) {
    return (
      <div className="min-h-screen pt-32 px-6 text-center text-text-bronze animate-fade-in flex flex-col items-center justify-center">
        <h2 className="text-3xl font-bold uppercase tracking-tight mb-4">404 // Projet introuvable</h2>
        <button
          onClick={() => navigate('/')}
          className="px-6 py-3 bg-primary text-text-bronze uppercase text-sm font-mono font-bold tracking-tight hover:bg-primary-hover transition-colors rounded"
        >
          Retour à l'accueil
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen pt-36 pb-32 px-6">
      <div className="max-w-6xl mx-auto space-y-8 md:space-y-24">
        ...
        <FadeIn className="space-y-12">
          <button
            ...
            className="group px-4 py-2 bg-transparent border border-border text-text-secondary text-sm font-mono font-bold uppercase tracking-tight rounded hover:border-border hover:text-text-bronze hover:bg-surface/80 transition-all flex items-center gap-2 w-fit leading-none"
          >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-end">
            <div className="space-y-6">
              <div className="flex flex-col items-start gap-2 sm:flex-row sm:items-center sm:gap-3">
                ...
                <span className="text-sm font-mono uppercase text-text-secondary">
                  {project.category}
                </span>
              </div>
              <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold text-text-bronze tracking-tighter uppercase leading-[0.9]">
                {project.title}
              </h1>
            </div>
            ...
            <div className="flex flex-wrap gap-2 justify-start lg:justify-end">
              {project.tags.map((tag) => (
                <span key={tag} className="text-xs font-mono uppercase tracking-tight px-2 py-1 border border-border text-text-secondary rounded">
                  {tag}
                </span>
              ))}
            </div>
        ...
        <FadeIn delay={200}>
          <div className="w-full border border-border bg-surface shadow-2xl relative group rounded overflow-hidden">
            <img
              src={project.image}
        ...
        <FadeIn delay={300}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 border-y border-border py-6 md:py-16">
            <div className="space-y-4">
              <div className="flex items-center gap-3 mb-2">
                <span className="text-sm font-mono text-text-muted">01</span>
                <h3 className="text-base font-bold text-text-bronze uppercase tracking-tight">Contexte</h3>
              </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 mb-2">
                <span className="text-sm font-mono text-text-muted">02</span>
                <h3 className="text-base font-bold text-text-bronze uppercase tracking-tight">Mission</h3>
              </div>
              <p className="text-text-secondary leading-relaxed text-base">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 mb-2">
                <span className="text-sm font-mono text-text-muted">03</span>
                <h3 className="text-base font-bold text-text-bronze uppercase tracking-tight">Challenges</h3>
              </div>
              <p className="text-text-secondary leading-relaxed text-base">
        ...
        {project.fullDetails.role && (
          <FadeIn delay={200}>
            <div className="bg-surface p-6 md:p-12 border border-border rounded">
              <div className="flex flex-col md:flex-row gap-12 items-start">
                <div className="md:w-64 shrink-0">
                  <span className="text-primary font-mono text-sm">// RÔLE</span>
                  <h3 className="text-2xl font-bold text-text-bronze uppercase tracking-tight mt-2">Mon Rôle</h3>
                </div>
        ...
        <div className="space-y-8 md:space-y-16">
          <FadeIn>
            <div className="flex flex-col gap-4 max-w-2xl">
              ...
              <h2 className="text-3xl md:text-4xl font-bold text-text-bronze uppercase tracking-tight">Design & Solutions</h2>
            </div>
          </FadeIn>
          ...
          {project.fullDetails.solutionBlocks ? (
              ...
              {project.fullDetails.solutionBlocks.map((block, index) => {
                if (block.type === 'text') {
                  ...
                  const containerClasses = block.preserveOriginal
                    ? "border border-border rounded overflow-hidden flex justify-center"
                    : "border border-border bg-surface rounded overflow-hidden";
                  const imgClasses = block.preserveOriginal
                    ? "h-auto object-contain max-w-full"
                    : "w-full h-auto object-cover";
                  const captionClasses = block.preserveOriginal
                    ? "flex items-center justify-center gap-2 text-sm font-mono uppercase text-text-muted mt-2"
                    : "flex items-center gap-2 text-sm font-mono uppercase text-text-muted mt-2";
                  return (
                    <FadeIn key={index} delay={100}>
            ...
            <div className="space-y-8">
              <FadeIn>
                <div className="text-text-secondary leading-relaxed text-lg whitespace-pre-line max-w-4xl border-l-2 border-border pl-6">
              ...
              {project.fullDetails.solutionImages?.map((img, index) => (
                <FadeIn key={index} delay={100}>
                  <div className="border border-border bg-surface mb-4 rounded overflow-hidden">
                    <img src={img.url} alt={img.caption} className="w-full h-auto object-cover" />
                  </div>
                  {img.caption && <p className="text-sm font-mono uppercase text-text-muted mb-12">// {img.caption}</p>}
                </FadeIn>
              ))}
            </div>
          )}
        ...
        {/* 5. KPI & Résultats */}
        <FadeIn delay={100}>
          <div className="border border-border bg-surface p-6 md:p-12 rounded">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
              <h3 className="text-2xl font-bold text-text-bronze uppercase tracking-tight">Impact & Résultats</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {project.fullDetails.kpi?.map((kpi, index) => (
                <div key={index} className="flex flex-col gap-3">
                  <span className="text-4xl font-bold text-text-main font-mono select-none opacity-80">0{index + 1}</span>
                  <div className="h-px w-full bg-border"></div>
                  <p className="text-text-bronze font-medium text-base uppercase tracking-wide leading-relaxed">
                    {kpi}
                  </p>
                </div>
              ))}
      ...
      {isVideoModalOpen && project.fullDetails.videoUrl && (
        <div
          className="fixed inset-0 z-[100] flex items-center justify-center animate-fade-in bg-background/95 backdrop-blur-xl cursor-pointer"
          ...
          >
            ...
            <div className="absolute top-0 left-0 right-0 p-6 md:p-8 flex justify-between items-center z-[110]">
              <div className="flex items-center gap-3">
                ...
                <h4 className="text-text-bronze font-bold uppercase tracking-tight hidden md:block">{project.title}</h4>
              </div>

              {/* Bouton Fermer Explicite - Toujours Visible */}
              <button
                onClick={() => setIsVideoModalOpen(false)}
                className="flex items-center gap-3 px-6 py-3 bg-surface text-text-bronze rounded border border-border hover:bg-primary hover:border-primary transition-all group shadow-[0_0_20px_rgba(0,0,0,0.5)]"
              >
            ...
            <div className="w-full h-full max-w-4xl max-h-[85vh] p-4 flex items-center justify-center z-[105]">
              <div className="w-full h-full max-w-md aspect-[9/16] shadow-[0_0_100px_rgba(39,160,108,0.2)] rounded-2xl overflow-hidden border border-border bg-black">
                <iframe
